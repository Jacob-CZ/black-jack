...
export default function Game({params}: {params: {gameId: string}}) {
    ...
    const [desiredCards, setDesiredCards] = useState<card[]>([])
    const [desiredPlayerIndex, setDesiredPlayerIndex] = useState<number[]>([0])
    const [playerIndex, setPlayerIndex] = useState<number[]>([0])
    const [dealerIndex, setDealerIndex] = useState<number>(0)
    const [playIndex, setPlayIndex] = useState<number>(0)
    const [desiredIndex, setDesiredIndex] = useState<number>(0)
    useEffect(() => {
        if (playIndex !== 0) {
            return
        }
        beginGame()
        setInterval(() => {
            console.log(cards.length, playIndex, desiredIndex)
        }, 1000)
    }, [])
    ...
    useEffect(() => {
        if (desiredIndex > playIndex){
            delay(500).then(() => {
                setPlayIndex(playIndex + 1)
            })
        }
    }, [desiredIndex, playIndex])

    useEffect(() => {
        console.log(playerIndex, desiredPlayerIndex)
        if (desiredPlayerIndex.length === 0) {
            return
        }
        if (playerIndex.length < desiredPlayerIndex.length) {
            setPlayerIndex([...playerIndex, 0])
        }
        for(let i = 0; i < desiredPlayerIndex.length; i++){
            console.log(playerIndex, desiredPlayerIndex)
            if (playerIndex[i] < desiredPlayerIndex[i]){
                const localIndex = playerIndex
                localIndex[i] += 1
                    console.log("localIndex: " + localIndex)
                setPlayerIndex(localIndex)
            }
        }

    },[desiredPlayerIndex, playerIndex])
    ...
    const beginGame = async () =>{
        ...
        cards.splice(0, 3, ...firstCards);
        console.log(cards)
        setDesiredCards(cards)
        setDesiredPlayerIndex([3])
        setDesiredIndex(3)
